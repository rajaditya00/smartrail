    ...
    <div className="space-y-6">
      <div className="relative flex flex-col">
        <CoachSelector
          ...
          onLiveClick={(coach) => {
            setSelectedCoach(coach);
            document.getElementById('exchange-hub')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
          }}
        />
        ...
        {incomingRequests.length > 0 && (
          <div ref={incomingRef} className="space-y-3 mb-6 bg-blue-50/50 p-4 rounded-3xl border border-blue-100">
            ...
            {incomingRequests.map((request: ExchangeData) => {
              ...
              return (
                <div key={request.exchangeId} className={`bg-white p-3 rounded-2xl border-l-[4px] shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-in slide-in-from-top-2 transition-all duration-300 ${shouldHighlight ? 'border-orange-500 ring-4 ring-orange-400/20 scale-[1.02]' : 'border-blue-500'
                  }`}>
                  <div className="flex-1 min-w-0 w-full">
                    <div className="flex flex-col gap-0.5 mb-1">
                      ...
                      <h3 className="font-black text-slate-800 text-sm break-words leading-tight">
                        {request.requester?.name || "Passenger"}
                      </h3>
                      {(request.requester?.sourceStation || request.requester?.destinationStation) && (
                        <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">
                          {request.requester?.sourceStation || 'Unknown'} → {request.requester?.destinationStation || 'Unknown'}
                        </p>
                      )}
                    </div>
                ...
                return (
                  <div className="space-y-3 mb-6 bg-blue-50/50 p-4 rounded-3xl border border-blue-100">
                    ...
                    {filteredPeers.map((peer, idx) => {
                      ...
                      return (
                        <div key={peer.socketId || idx} className={`group flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-2 p-4 bg-white rounded-2xl border-2 transition-all duration-300 ${isHighlighted
                          ...
                          <div className="flex items-start sm:items-center gap-3 w-full">
                            ...
                            <div>
                              ...
                              {(peer.sourceStation || peer.destinationStation) && (
                                <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-0.5 mb-1">
                                  {peer.sourceStation || 'Unknown'} → {peer.destinationStation || 'Unknown'}
                                </p>
                              )}
                          ...
                          <div className="flex w-full sm:w-auto justify-end sm:shrink-0 mt-2 sm:mt-0">
                            <button
                              onClick={() => {
                              ...
                              className={`px-4 py-2 rounded-xl text-[10px] font-black tracking-wider uppercase transition-all shadow-lg shadow-blue-900/10 flex items-center justify-center gap-2 w-full sm:w-auto ${sentRequests.includes(peer.socketId)
